...
class HumanSizeTests(unittest.TestCase):
    ...
    def test_exact_kb_boundary(self) -> None:
        # 1024 flips out of the bytes branch into KB with .1f formatting.
        self.assertEqual(human_size(KB), "1.0 KB")
    ...
    def test_above_pb_clamps_to_pb(self) -> None:
        # 1024 ** 6 has no larger unit defined; it should still render as PB.
        result = human_size(1024**6)
        self.assertTrue(result.endswith(" PB"), result)
        self.assertEqual(result, "1024.0 PB")

    def test_rounding_one_decimal(self) -> None:
        # 1124 / 1024 ≈ 1.0976... → rounded to 1.1 KB by .1f
        self.assertEqual(human_size(1124), "1.1 KB")
