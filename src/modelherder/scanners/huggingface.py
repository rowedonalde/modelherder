...
def _decode_model_name(dir_name: str) -> str | None:
...
def scan_huggingface(hub_dir: Path = DEFAULT_HF_HUB) -> list[ModelEntry]:
    ...
    for model_dir in sorted(hub_dir.iterdir()):
        if not model_dir.is_dir():
            continue
        name = _decode_model_name(model_dir.name)
        if name is None:
            continue
...
def collect_known_paths(hub_dir: Path = DEFAULT_HF_HUB) -> set[Path]:
    """Return resolved blob and snapshot paths so the stray scan can skip them."""
    paths: set[Path] = set()
    if not hub_dir.exists():
        return paths
    try:
        paths.add(hub_dir.resolve())
    except OSError:
        pass
    return paths
